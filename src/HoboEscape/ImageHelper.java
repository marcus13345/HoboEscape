...
public class ImageHelper {
	...
	public static BufferedImage creatImageWithStripes(int width, int height,
			Color c) {
	...
	public static BufferedImage getScaledImage(BufferedImage image, double scale)
		...
		return bilinearScaleOp.filter(image,
				new BufferedImage((int) (imageWidth * scale),
						(int) (imageHeight * scale), image.getType()));
	}

	public static BufferedImage createImageCloud(int width, int height,
			Color color) {
		return createImageCloud(width, height,
				0xFF000000 | (color.getRed() << 16) | (color.getGreen() << 8)
						| (color.getBlue() << 0));
	}

	public static BufferedImage createImageCloud(int width, int height,
			int color) {
		return createImageCloud(width, height, 0x000000FF & (color >> 16),
				0x000000FF & (color >> 8), 0x000000FF & (color));
	}

	public static BufferedImage createImageCloud(int width, int height, int r,
			int g, int b) {
	...
	public static BufferedImage createImageCloud(int width, int height,
		...
		// end wtf code
		BufferedImage img = new BufferedImage(DATA_SIZE, DATA_SIZE,
		...
		BufferedImage _return = new BufferedImage(width, height,
				BufferedImage.TRANSLUCENT);
		_return.getGraphics().drawImage(img, 0 - (img.getWidth() / 8),
				0 - (img.getHeight() / 8), null);
		return _return;
