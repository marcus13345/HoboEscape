...
public class Button {
	...
	public Button(Room parent, int id, Color c, int x, int y, int width, int height, String text, int leftPadding, int bottomPadding) {
		this(parent, id, c, x, y, width, height, text, leftPadding, bottomPadding, true);
	}
	
	public Button(Room parent, int id, Color c, int x, int y, int width, int height, String text, int leftPadding, int bottomPadding, boolean active) {
		room = parent;
		...
		normal = ImageCreator.creatImageColorWithNoise(width, height, c, 2);
		hover = ImageCreator.creatImageColorWithNoise(width, height, c.brighter(), 2);
		down = ImageCreator.creatImageColorWithNoise(width, height, c.darker(), 2);
	...
	public void poll() {
		...
		if (active) {
			...
			// we clicked it
			if (clickLevel == 0) {
				if (inBoundingBox(HoboEscape.getMouseX(room), HoboEscape.getMouseY(room)) && !HoboEscape.mouse) {
					clickLevel = 1;
				}
				...
				if (inBoundingBox(HoboEscape.getMouseX(room), HoboEscape.getMouseY(room)) && HoboEscape.mouse) {
					...
					if (!inBoundingBox(HoboEscape.getMouseX(room), HoboEscape.getMouseY(room))) {
						clickLevel = 0;
					}
				}
				...
				} else if (!inBoundingBox(HoboEscape.getMouseX(room), HoboEscape.getMouseY(room))) {
					clickLevel = 3;
				}
